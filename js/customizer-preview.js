...
( function( $ ) {
	...
	// Logo
	wp.customize( 'inku_logo', function( value ) {
		value.bind( function( to ) {
			if ( to != "" ) {
				var logo = '<img src="' + to + '" />';
    ...
    */
		//Featured Color
		wp.customize( 'inku_hero_color', function( value ) {
			value.bind( function( to ) {
				$( '.btn-ql, .pagination li.active a, .pagination li.active a:hover, .wpb_wrapper .products .product-category h3, .btn-ql:active, .btn-ql.alternative:hover, .btn-ql.alternative-white:hover, .btn-ql.alternative-gray:hover, .hero_bck, .inku-nav-btn:hover, .inku-nav-btn:active, .cd-popular .cd-select, .no-touch .cd-popular .cd-select:hover, .pace .pace-progress, .woocommerce .products .product .add_to_cart_button:hover, #ql_woo_cart .widget_shopping_cart_content a.button.checkout' ).css( {
						'background-color': to
				} );
		...
		//Headings Color
		wp.customize( 'inku_headings_color', function( value ) {
			value.bind( function( to ) {
		...
		//Logo Color  
		wp.customize( 'inku_logo_color', function( value ) {
			value.bind( function( to ) {
		...
		//Text Color
		wp.customize( 'inku_text_color', function( value ) {
			value.bind( function( to ) {
				$( 'body' ).css( {
						'color': to
				} );
		...
		//Link Color
		wp.customize( 'inku_link_color', function( value ) {
			value.bind( function( to ) {
				$( 'a' ).css( {
						'color': to
				} );
		...
		wp.customize( 'inku_content_background_color', function( value ) {
			value.bind( function( to ) {
		...
		wp.customize( 'inku_footer_background', function( value ) {
			value.bind( function( to ) {
		...
		wp.customize( 'inku_header_bck_color', function( value ) {
			value.bind( function( to ) {
				$( '#header, .single-product #header, .inku-sidenav #header, .top-bar, .single-product .top-bar' ).css( {
						'background-color': to
				} );
			} );
		} );
		...
		//Header Lines Color
		wp.customize( 'inku_header_lines_color', function( value ) {
			value.bind( function( to ) {
				$( '.inku-sidenav #header .ql_cart-btn, #jqueryslidemenu ul.nav > li, .inku-sidenav #header .logo_container, .ql_cart-btn, #header, .single-product #header, .top-bar, .inku-sidenav #header, .logo_container::before, .inku-header-2 #header .logo_container::before, .inku-header-2 #header #ql_nav_collapse #jqueryslidemenu ul.nav > li, .inku-header-2 #header #ql_nav_collapse #jqueryslidemenu ul.nav > li:last-child, .inku-header-2 #header .ql_cart-btn' ).css( {
						'border-color': to
				} );
			} );
		} );
    ...
    */
	//Shop Layout
	wp.customize( 'inku_shop_layout', function( value ) {
		value.bind( function( to ) {
			if ( 'masonry' == to ) {
	...
	//Shop Columns
	wp.customize( 'inku_shop_columns', function( value ) {
		value.bind( function( to ) {
    ...
    */
    //Background Color
	wp.customize( 'inku_site_background_color', function( value ) {
		value.bind( function( to ) {
			$( 'body' ).css( {
					'background-color': to
			} );
