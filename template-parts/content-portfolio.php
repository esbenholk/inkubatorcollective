...
$terms_to_print = 'inku-firstload ';
$image_print = '';

if ( has_post_thumbnail() ) {
	$portfolio_image = wp_get_attachment_image_src( get_post_thumbnail_id(), 'inku_portfolio' );
	$portfolio_image_2x = wp_get_attachment_image_src( get_post_thumbnail_id(), 'inku_portfolio_2x' );
	$inku_portfolio_item_layout = rwmb_meta( 'inku_portfolio_item_layout' );
	if( $portfolio_image[2] > $portfolio_image[1] ){
		$terms_to_print .= 'layout-portrait';
	}else if ( $inku_portfolio_item_layout ) {
		$terms_to_print .= $inku_portfolio_item_layout;
	}else{
